...
| Run Script      | `./file.sh`             | `./myapp.sh`                                  |
| Comment         | `# comment`             | `echo "Datta" # inline comment`               |
| Variable        | `VAR="value"`           | `NAME="DevOps"`                               |
| Use Variable    | `$VAR`                  | `echo $NAME`                                  |
| Read Input      | `read VAR`              | `read USER`                                   |
| Arguments       | `$1 $2 $# $@ $?`        | `./myapp.sh arg1`                             |
...
|  AND            | `cmd1 && cmd2`          | `mkdir test1 && cd test1`                     |
...
## 2.Running a script:
- `chmod +x myapp.sh`: Make the script executable.
- `./myapp.sh`: Run the script. The system uses the shebang (`#!/bin/bash`) to choose the shell. If there is no shebang, it uses the current shell.
- `bash myapp.sh`: Run the script directly with Bash, ignoring the shebang.
...
- **Exmaple**:
    ...
    echo "Devops"  # This prints Devops(Inline comment)
    ```
...
- **Quoting**:
 - **Double quotes " "**: Variable is replaced with its value.
    ```bash
    echo "Hello $NAME"   # Prints Hello Datta
    ```
...
- $?: Last command status
 ...
 ```bash
    ...
    ls /Datta                              # This directory does not exist
    ...
    ./myapp.sh                             # $0 = myapp.sh, no arguments
    ./myappt.sh Datta                      # $1 = Datta, $# = 1, $@ = Datta
    ./myapp.sh a b c                       # $1 = a, $# = 3, $@ = a b c
```
...
- **Example**: Prints user IDs for root,ubuntu,and sanket
    ```bash
    for USER in root ubuntu Datta; do
        id "$USER"
    done
    ```
...
- **Example**:
    ```bash
        echo "Devopos"     | tr 'a-z' 'A-Z'   # convert to uppercase
        echo "Devops 123" | tr -d '0-9'  # delete digits
    ```
...
- **Example**:
    ...
    | Command              | Meaning                               |
    | -------------------- | ------------------------------------- |
    | `ls devops_file.txt` | Tries to list the file                |
